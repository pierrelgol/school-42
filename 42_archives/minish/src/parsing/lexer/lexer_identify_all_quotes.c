...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   lexer_identify_all_quotes.c                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: pollivie <pollivie.student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/06 15:01:04 by pollivie          #+#    #+#             */
/*   Updated: 2024/07/06 15:01:04 by pollivie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (!it_end(it))
	{
		...
		if (token && token_get_kind(token) == KIND_NO_KIND)
		{
			if (token->len == 1 && token->string[0] == '"')
				token_set_kind(token, KIND_DQUOTE);
			else if (token->len == 1 && token->string[0] == '\'')
				token_set_kind(token, KIND_QUOTE);
		}
		it_advance(it);
	}
	it_restore(it);
